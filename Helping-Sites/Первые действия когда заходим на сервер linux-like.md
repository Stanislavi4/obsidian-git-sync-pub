...
sudo --list - показывает список всех доступных пакетов и привилегий (RWXD), которые можно выполнить с помощью `sudo`. Определения того, какие действия вы можете выполнять с помощью суперпользовательских прав.

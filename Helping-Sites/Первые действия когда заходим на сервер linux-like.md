...
chown -  используется для изменения владельца и группы файлов или директорий. Она принимает два аргумента - номер уникального идентификатора (UID) пользователя, который станет новым владельцем, и номер GID - группового идентификатора (GID), который будет использован для новой группы.
```
